...
@section('content')
    <div class="col-md-12">
        <h3>Editar Homilia</h3>
        <div class="alert alert-danger" id="erro"><i class="fa fa-exclamation-triangle"></i> Verifique os campos
        </div>
        <div class="well">
            <form method="post" action="{{route('homilia.atualiza',$homilia->id)}}" enctype="multipart/form-data">
                {{csrf_field()}}

                <div class="form-group col-md-12" id="vtitulo">
                    <label class="control-label" for="Titulo">Titulo</label>
                    <input id="titulo" name="titulo" value="{{$homilia->titulo}}" min="3" type="text" placeholder="Informe um titulo"
                           class="form-control input-md">
                </div>

                <div class="form-group col-md-12" id="vurl">
                    <label class="control-label" for="url">URL</label>
                    <input id="url" name="url" value="{{$homilia->url}}" min="3" type="text" placeholder="Informe uma url"
                           class="form-control input-md">
                </div>
